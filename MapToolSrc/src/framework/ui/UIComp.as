package framework.ui
{
	...
	public class UIComp extends GComponent
	{
		...
		{
			var children:Vector.<GObject> = view.getChildren();
			if (childCompDic == null) childCompDic = new Dictionary();
			if (_objTapMap == null) _objTapMap = new Dictionary();
			for (var i:int = 0; i < children.length; i++)
			{
				...
				if (item is GComponent && item.packageItem)
				{
					...
					if(hasOwnProperty("_tap_" + itemName)){//注册对象点击事件
						item.addClickListener(this["_tap_" + itemName]);
						_objTapMap["_tap_" + itemName] = item;
					}
		...
		public function __dispose(): void
		{
			...
			if(_objTapMap){
				for(var tapKey:String in _objTapMap){
					var tapItem:GObject = _objTapMap[tapKey];
					tapItem.removeClickListener(this[tapKey]);
				}
			}
